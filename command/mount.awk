...
function mount_filter(line)
{
	...
	if (match(line, ": mount -f\\]$")) {
		allow_regx="option not supported or applicable on this architecture or kernel|^\\s*[a-f0-9]+ +[a-f0-9]+ ";
		next;
	}

	#eg:c72c4008
	if (match(line, ": mount -i\\]$")) {
		allow_regx="option not supported or applicable on this architecture or kernel|^\\s*[a-f0-9]+";
		next;
	}
}